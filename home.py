'''
This file contains the code for the home window of the application.
The home window displays all logged-in users in a table format.
The data is fetched from the database and displayed in a Treeview widget.
'''
import datetime
...
class HomeWindow(tk.Tk):
    '''
    Class: HomeWindow
    Function: GUI for the home window, displaying all logged-in users
    '''
    def __init__(self):
        super().__init__()
        self.title("Home")
    ...
    def populate_table(self):
        ...
        function: Populate the Treeview widget with data from the database
        '''
        ...
        for row in rows:
            ...
            if login_time_str:
                ...
                login_time_obj = datetime.datetime.strptime(login_time_str, '%Y-%m-%d %H:%M:%S.%f')
